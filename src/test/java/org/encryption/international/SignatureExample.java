...
import org.encryption.international.util.SignatureUtils;
...
public class SignatureExample {
    private final String INPUT_STR = "test secure test securetest secure test securetest secure test securetest secure test securetest secure test securetest secure test securetest secure test securetest secure test secure";
    ...
    private static String publicKeyString =
            "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQC3kjdBNSzLPdI85vuEZmX+YTEl" +
    ...
    public void test() {
        ...
        String sign3 = SignatureUtils.getSignRSA(INPUT_STR, privateKeyString, SignAlgorithm.SHA1WithRSA);
        System.out.println("签名：" + sign3 + "\n length:" + sign3.length());
        System.out.println("验证：" + SignatureUtils.verify(INPUT_STR, publicKeyString, sign3, SignAlgorithm.SHA1WithRSA));

        System.out.println("=====使用指定方法，指定加密key,指定编码,指定加签算法==================");
        String sign4 = SignatureUtils.getSignRSA(INPUT_STR, privateKeyStringHex, SignAlgorithm.SHA1WithRSA, EncType.BASE64, EncType.HEX);
        System.out.println("签名：" + sign4 + "\n length:" + sign4.length());
        System.out.println("验证：" + SignatureUtils.verify(INPUT_STR, publicKeyString, sign4, SignAlgorithm.SHA1WithRSA, EncType.BASE64, EncType.BASE64));
    }
}